...
storage (disk files, or in general case, a random-access :term:`stream`). Keys are
stored sorted in the database, and besides efficient retrieval by a key
value, a database also supports efficient ordered range scans (retrieval
of values with the keys in a given range). On the application interface
side, BTree database work as close a possible to a way standard :class:`dict`
type works, one notable difference is that both keys and values must
be  :class:`bytes` objects (so, if you want to store objects of other types, you
need to serialize them to  :class:`bytes` first).
