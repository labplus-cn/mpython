...
class _APDS9960:
    ...
    # start the light (R/G/B/Ambient) sensor 启动环境光(R/G/B/Ambient)传感器
    ...
    # processes a gesture event and returns best guessed gesture 处理一个手势事件并返回最佳猜测的手势  
    def readGesture(self):
        fifo_level = 0
        bytes_read = 0
        fifo_data = []

        # make sure that power and gesture is on and data is valid 确保电源和手势是打开的，数据是有效的  
        if not (self.getMode() & 0b01000001) or not self.isGestureAvailable():
            return APDS9960_DIR_NONE

        # keep looping as long as gesture data is valid 只要手势数据有效，就保持循环
        while(self.isGestureAvailable()):
            # read the current FIFO level 读取当前的FIFO级别
            fifo_level = self._read_byte_data(APDS9960_REG_GFLVL)

            # if there's stuff in the FIFO, read it into our data block 如果有数据在FIFO，读入我们的数据块  
            if fifo_level > 0:
                fifo_data = []
                for i in range(0, fifo_level):
                    fifo_data += self._read_i2c_block_data(APDS9960_REG_GFIFO_U, 4)

                # if at least 1 set of data, sort the data into U/D/L/R 如果至少有一组数据，则将数据排序为U/D/L/R  
                if len(fifo_data) >= 4:
                    ...
                    # filter and process gesture data, decode near/far state 过滤和处理手势数据，解码近/远状态  
    ...
    def decodeGesture(self):
        """Determines swipe direction or near/far state. 确定滑动方向或近/远状态。
        """

        # return if near or far event is detected 如果检测到近或远事件，则返回
    ...
    def clearProximityInt(self):
        """Clears the proximity interrupt. 清除近距离中断。"""
    ...
    def setGestureMode(self, enable):
        """Turns gesture-related interrupts on or off. 打开或关闭与手势相关的中断。
