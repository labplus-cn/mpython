...
class REPL:
    ...
    def read_until(self, min_num_bytes, ending, timeout=20, data_consumer=None):
        # if data_consumer is used then data is not accumulated and the ending must be 1 byte long ;如果使用了data_consumer，则不会累计数据，且结尾必须为1字节  
        ...
        while True:
            if data.endswith(ending):
                ...
                if timeout is not None and timeout_count >= 100 * timeout:
                    break
                time.sleep(0.01)
            # if timeout_count%100 == 0 and timeout_count != 0:
            #     print(timeout_count)
        return data

    def enter_raw_repl(self, timeout=20, soft_rest=False):
        # ctrl-C twice: interrupt any running program 中断任何正在运行的程序,发送两次ctrl-C
        self.serial.write(b"\r\n\x03\x03")
        ...
        if soft_rest:
            ...
            if not data.endswith(b"soft reboot\r\n"):
                raise RemoteException("could not enter raw repl")
            # By splitting this into 2 reads, it allows boot.py to print stuff, 通过将它分成两次读取，它允许boot.py打印
    ...
    def exec_raw(self, command, timeout=100, data_consumer=None):#timeout=20
