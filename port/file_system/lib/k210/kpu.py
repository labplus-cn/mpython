...
class KPU:
    ...
    def forward(self, kpu_net, img, out_index=0):
        ...
        # print(kpu_net.name,type(kpu_net.name))
        self.repl.exec_(cmd)
        return eval(self.repl.eval("fmap[:]"))

    def set_outputs(self,kpu_net, out_idx, width, height, channel):
        if not isinstance(kpu_net, KPU_net):
            raise TypeError("argument #1 must be KPU_net object")
        cmd = "fmap=KPU.set_outputs({0},{1},{2},{3},{4})".format(
            kpu_net.name, out_idx,width,height,channel)
        # print('---------------------------------------------------------')
        self.repl.exec_(cmd)
        return eval(self.repl.eval("fmap"))
