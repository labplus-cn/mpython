...
class Image:
    ...
    def find_blobs(self, thresholds, invert=False, roi=None, x_stride=2, y_stride=1, area_threshold=10, pixels_threshold=10, merge=False, margin=0):
        ...
        # print(self.repl.eval(cmd))
        # print(type(self.repl.eval(cmd)))
        # print(eval(self.repl.eval(cmd)))
        # print(type(eval(self.repl.eval(cmd))))
        return eval(self.repl.eval(cmd))

    def find_apriltags(self):
        cmd = "{0}.find_apriltags()" .format(self.name)
        return eval(self.repl.eval(cmd))
